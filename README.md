...
> IDE multi-session 自动点击器 — 替你在 VS Code、Kiro、Antigravity、Cursor 等 IDE 里自动点按钮、切标签、确认弹窗。

## 一句话

同时开着 N 个 IDE 窗口跑任务，每个都需要隔几分钟点一下 **Run** / **Accept** / **Continue**？
AutoClicker 帮你盯着，看到按钮就点，省下反复切窗口的重复劳动。
...
### 模式 1：OCR 文字识别（通用）
自动识别屏幕上的 **Run / Accept / Continue / Yes / Confirm** 等按钮文字，无需事先截图。
...
```
auto_confirm/          ← 项目根目录
├── clicker.py         # 主程序（录制 + 循环点击）
├── capture_template.py # 图像模板截图工具
...
- macOS（目前），Windows/Linux 可改截图 API
- Python 3.9+
- Tesseract OCR
- 屏幕录制 + 辅助功能权限
